...
// Bulk Download Action Bar (WhatsApp style)
const BulkDownloadBar = ({ selectedCount, selectedMessages, onClose, messages, authUser }) => {
  ...
  const downloadAllFiles = async () => {
    ...
    try {
      const filesToDownload = selectedMessages
        .map(id => messages.find(msg => msg.id === id))
        .filter(msg => msg && msg.file_url);
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-r from-[#6200B3] to-[#8b35d6] text-white p-4 shadow-2xl animate-slide-up">
      <div className="flex items-center justify-between max-w-7xl mx-auto">
        <div className="flex items-center gap-3">
          <div className="bg-white/20 rounded-full px-4 py-2">
            <span className="font-semibold text-lg">{selectedCount}</span>
          </div>
          <span className="text-sm opacity-90">item{selectedCount !== 1 ? 's' : ''} selected</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 bg-white/20 hover:bg-white/30 disabled:opacity-50 px-4 py-2 rounded-lg transition-all duration-200 backdrop-blur-sm"
          >
            <Download size={18} />
...
const ChatArea = () => {
  ...
  const getFileIcon = (type) => {
    ...
    if (type.includes("pdf")) 
      return <img className="w-8 h-full" src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/87/PDF_file_icon.svg/1667px-PDF_file_icon.svg.png" alt="pdf" />;
    if (type.includes("word") || type.includes("msword"))
      return <img className="w-8 h-full" src="https://cdn-icons-png.flaticon.com/512/9496/9496487.png" alt="word" />;

    if (type.includes("excel") || type.includes("spreadsheet"))
      return <img className="w-8 h-full" src="https://www.freeiconspng.com/thumbs/excel-icon/excel-icon-12.png" alt="excel" />;
    if (type.includes("zip") || type.includes("compressed"))
      return <img className="w-8 h-full" src="https://cdn-icons-png.flaticon.com/512/2656/2656401.png" alt="zip" />;
    if (type.includes("video") || type.includes("mp4"))
      return <img className="w-8 h-full" src="https://static.vecteezy.com/system/resources/thumbnails/010/161/430/small/3d-rendering-blue-clapperboard-with-play-icon-isolated-png.png" alt="video" />;
    return <File size={24} />;
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-3 inter-large newbg">
        ...
        <dialog open className="modal">
          <div className="modal-box flex flex-col items-center sm:w-1/2 w-full bg-white">
            ...
            <img
              ...
              alt="preview"
            />
            <button className="border-2 border-[#6200B3] w-[80%] mt-2 cursor-pointer text-[#6200B3] p-1 px-4 rounded-md hover:bg-[#6200B3] hover:text-white transition-colors">
              Download
            </button>
          </div>
      ...
      {messages?.map((message, index) => {
        ...
        const isSelected = selectedChatIds.includes(message.id);
        ...
        return (
          <div
            ...
            className={`chat ${isMine ? "chat-end" : "chat-start"} transition-all duration-200 ${
              isSelected ? "bg-[#eed8ff] rounded-lg" : ""
            } ${isSelecting === true ? "cursor-pointer rounded-xl hover:bg-[#e4e4e4]" : ""}`}
          >
            <div className="chat-header text-black flex items-center mb-1 gap-2">
              {isSelecting === true && (
                ...
                >
                  {isSelected ? (
                    <CheckCircle2 size={20} className="text-[#6200B3]" />
                  ) : (
              ...
              {hoveredMessageId === messageKey && !isSelecting && (
            ...
            >
              ...
              {hasFile && isImage && (
                ...
                >
                  <img
                    ...
                    className="w-32 h-32 object-cover rounded-lg mb-2 cursor-pointer hover:opacity-80 transition-opacity"
                    alt="chat-img"
                  />
                </button>
              )}
              ...
              {hasFile && !isImage && !isAudio && message.file_type && (
                <div className="flex flex-col gap-2">
                  ...
                  <button
                    ...
                    className="cursor-pointer border rounded-lg py-1 px-2 text-sm hover:bg-black/10 transition-colors flex items-center justify-center gap-2"
                  >
                    <Download size={14} />
                    Download
                  </button>
      ...
      {/* Bulk Download Bar */}
      {isSelecting && selectedChatIds.length > 0 && (
        <BulkDownloadBar
          selectedCount={selectedChatIds.length}
          selectedMessages={selectedChatIds}
